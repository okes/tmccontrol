...
export class HistoriaFinanzas extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    const { cuentalist, gananciaslist, gastoslist } = this.state;
    const newobj = { };
    let sendok = false;
  ...
  loadAllData = () => {
    ...
    getLists(actionFetch.goGetFinanzasGananciasParam(idlocal, 1));
    getLists(actionFetch.goGetFinanzasGastosParam(idlocal, 2));
  }
  ...
  loadinfo = (props, newobj) => {
    ...
    _objestate.list = this.getListFinanzas(props.gananciaslist, props.gastoslist);
    ...
    return (
      <div key={item.date} >
        ...
        {item.list.map((itemii, i) => {
          ...
          return (
            <Row key={itemii.obj.idvalor} className={classbg}>
              <Col xs="7" sm="7" lg="7" className="p-1">
  ...
  renderUserList = (): Element<any> => {
    ...
    const toggleDeleteAlertaOK = () => {
      if (modaldeleteItem) {
        const typeres = modaldeleteItem.type;
        this.setState({ sendingload: true });

        if (modaldeleteItem.obj.idtype === 0 && modaldeleteItem.obj.extra !== undefined) {
          goSendData({
            service: 'aws',
            method: 'valoresdeletereverse',
            query: {
              cuotan: modaldeleteItem.obj.extra.cuotan,
              idpv: modaldeleteItem.obj.extra.idpv,
            },
          })
            .then((res) => {
              ...
              if (typeres === 0) {
                ...
                goSendNotification({ msj: 'Gasto Eliminado!' });
                getLists(actionFetch.goGetFinanzasGastosParam(idlocal, 2));
              } else if (typeres === 1) {
                goDeleteData(actionFetch.goGetFinanzasGananciasDelete());
                getLists(actionFetch.goGetFinanzasGananciasParam(idlocal, 1));
                goSendNotification({ msj: 'Ganancia Eliminada!' });
              }
            })
            .catch((error) => {
              console.log(error);
          ...
          goSendData({
            service: 'aws',
            method: 'valoresdelete',
            query: {
              date: modaldeleteItem.obj.date,
              idtype: modaldeleteItem.obj.idtype,
            },
          })
            .then((res) => {
              toggleDeleteAlerta(true);
...
const mapStateToProps = ({ utilfetch, local }: Reducer) => {
  let _infoload = false;
  if (utilfetch.cuentas.data !== undefined && utilfetch.finanzas_ganancias.data !== undefined && utilfetch.finanzas_gastos.data !== undefined) { // eslint-disable-line max-len
    _infoload = true;
  }
  const props = {
    idlocal: local.id,
